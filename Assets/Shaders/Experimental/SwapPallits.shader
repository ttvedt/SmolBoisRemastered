...
Shader "Custom/SwapPallits"
{
	Properties
	{
		...
		[PerRendererData] _NormalMapTex("Normal Map Texture", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f IN) : COLOR
			{
				...
				float4 NormalpCoord = tex2Dlod(_NormalMapTex, float4(IN.texcoord.xy, 0.0, 0.0));

				float palSize = 1.0 / _PallitCount;   //determines how long a pallet is based on how many pallets there are based on length -Not height :(
				...
				return c * _Color;                 //(r=x coord g=y coord * pallet size) + (pallet size * shift-1) //Trying to understand its process
			}
		ENDCG
		}
	}
